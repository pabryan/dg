...
fig = make_subplots(rows=2, cols=2, row_heights=[0.8, 0.2], subplot_titles = (r"$\gamma(t) = (\cos(t), \sin(t)) \quad -\pi < t < \pi$", r"$\gamma(t) = (\cos(t^2), -\sin(t^2)) \quad 0 < t < \sqrt{2\pi}$"))
...
fig.update_xaxes(zeroline=False, showline=False, range=[-0.5, np.sqrt(2*np.pi) + 0.5], tickmode = "array", tickvals=np.linspace(0.0, np.sqrt(2*np.pi), 5), ticktext=[r"$0$", ".", ".", ".", r"$\sqrt{2\pi}$"], row=2, col=2)
...
fig.add_trace(go.Scatter(x=unit_circle_alt["x"], y=unit_circle_alt["y"], name="alternative unit circle", showlegend=False, line=dict(color=colors[1])), row=1, col=2)
fig.add_trace(go.Scatter(x=unit_circle_alt["ph"], y = np.zeros_like(unit_circle_alt["ph"]), showlegend=False, line=dict(color=colors[1])), row=2, col=2)
...
fig.add_annotation(arrowhead(t1, 0, 0, xref="x3", yref="y3", axref="x3", ayref="y3"), row=2, col=1)

fig.add_trace(go.Scatter(x=[x0], y=[y0], mode="markers", showlegend=False, marker=dict(color="black", size=10)), row=1, col=2)
fig.add_trace(go.Scatter(x=np.cos(arcph**2), y=-np.sin(arcph**2), showlegend=False, line=dict(color="black")), row=1, col=2)
fig.add_annotation(arrowhead(x1, y1, th1, xref="x2", yref="y2", axref="x2", ayref="y2"), row=1, col=2)

fig.add_trace(go.Scatter(x=[np.sqrt(2*np.pi)], y=[0.0], mode="markers", showlegend=False, marker=dict(color="black", size=10)), row=2, col=2)
fig.add_trace(go.Scatter(x=arcph, y=np.zeros_like(arcph), showlegend=False, line=dict(color="black")), row=2, col=2)
fig.add_annotation(arrowhead(p1, 0, -np.pi, xref="x4", yref="y4", axref="x4", ayref="y4"), row=2, col=2)
fig.update_yaxes(range=[-1, 1], row=2, col=2)

fig.update_layout(title_text="Parametrisations of the unit circle")

fig.write_html("../../docs/_includes/curves_unit_circle_plot.html", include_plotlyjs="cdn", include_mathjax=False, full_html=False)
